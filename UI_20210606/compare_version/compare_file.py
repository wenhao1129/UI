...
class MyWin(Ui_MainWindow, QMainWindow):
    ...
    def compare_version(self):
        ...
        while not self.TextBrowser_show.isFinished():
            ...
            QApplication.processEvents()
        time.sleep(3)
        if not os.path.exists('../version/temp_ecu_version.txt'):
            self.textBrowser.append('There is not a ECU version document')
        elif os.path.exists('../version/temp_ecu_version.txt'):
        ...
        while not self.TextBrowser_show.isFinished():
            ...
            QApplication.processEvents()
        time.sleep(3)
        if not os.path.exists('../version/temp_sca_version.txt'):
            self.textBrowser.append('There is not a SCA version document')
        elif os.path.exists('../version/temp_sca_version.txt'):
        ...
        while not self.TextBrowser_show.isFinished():
            ...
            QApplication.processEvents()
        time.sleep(3)
        if not os.path.exists('../version/temp_iris_camera.txt'):
            self.textBrowser.append('There is not a iris version document')
        elif os.path.exists('../version/temp_iris_camera.txt'):
        ...
        if ccu_version and ecu_version and sca_version and supernode_version:
           ...
           self.textBrowser.append('Version is different,pls double check')
           self.textBrowser.append('版本比对结束')
           testStatusText = 'fail'
           self.lineEdit_teststatus.setText(testStatusText)
