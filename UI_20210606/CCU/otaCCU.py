        ...
        if startResult == QMessageBox.No:
            ...
            if setupresult == 'fail':
                ...
                if startGingerserviceresult == 'fail':
                    ...
                    self.textBrowser.append('OTA test')
                    self.data = subprocess.Popen('../CCU/OTA.sh', stdout=subprocess.PIPE, stdin=subprocess.PIPE,
                                                 shell=True)
                    ...
                    time.sleep(3)
                    with open('tmp.txt', 'r') as OTAFile:
                        OTAfailkeyword1 = '1 passed, 1 failed'
                        OTAfailkeyword2 = '0 passed, 2 failed'
                        OTApasskeyword = '2 passed, 0 failed'
                        for tempsetup in OTAFile.readlines():
                            if OTAfailkeyword1 in tempsetup or OTAfailkeyword2 in tempsetup:
                                testStatusText = 'Fail'
                                self.lineEdit_teststatus.setText(testStatusText)
                                self.lineEdit_teststatus.setStyleSheet("background-color:red")
                                self.textBrowser.append('OTA失败')
                                ...
                                save_UI_logfile.MyWin.Self_logfile(self, testresult)
                                break
                            elif OTApasskeyword in tempsetup:
                                testStatusText = 'PASS'
                                self.lineEdit_teststatus.setText(testStatusText)
