...
class MyWin(Ui_MainWindow, QMainWindow):
    ...
    def IP128(self):
        ...
        self.data = subprocess.Popen('../setup_param/HostIP128.sh', stdout=subprocess.PIPE, stdin=subprocess.PIPE, shell=True)
        ...
        with open('tmp.txt', 'r') as IP200File:
            IP128failkeyword = '0 passed, 1 failed'
            IP128passkeyword = '1 critical test, 1 passed, 0 failed'
            for tempIP128 in IP200File.readlines():
                if IP128failkeyword in tempIP128:
                    testStatusText = 'Fail'
                    self.lineEdit_teststatus.setText(testStatusText)
                    self.lineEdit_teststatus.setStyleSheet("background-color:red")
                    self.textBrowser.append('IP128设置失败')
                    ...
                    break
                elif IP128passkeyword in tempIP128:
                    testStatusText = 'PASS'
                    self.lineEdit_teststatus.setText(testStatusText)
                    self.lineEdit_teststatus.setStyleSheet("background-color:green")
                    self.textBrowser.append('IP128设置成功')
                    with open('tmp.txt','r') as tmpfile:
                        with open('Ginger_UI_log.txt','w') as Gingerfile:
                            for line in tmpfile:
                                Gingerfile.write(line)
