           ...
           if setupresult == 'fail':
              ...
              if startGingerserviceresult == 'fail':
                 ...
                 save_UI_logfile.MyWin.Self_logfile(self, testresult)
                   ...
                   self.textBrowser.append('IMU test')
                   time.sleep(1)
                   self.data = subprocess.Popen('../collect_param/IMU.sh', stdout=subprocess.PIPE,
                   ...
                   time.sleep(3)
                   with open('tmp.txt', 'r') as IMUFile:
                       IMUfailkeyword1 = '1 passed, 1 failed'
                       IMUfailkeyword2 = '0 passed, 2 failed'
                       IMUpasskeyword = '2 passed, 0 failed'
                       for tempsetup in IMUFile.readlines():
                           if IMUfailkeyword1 in tempsetup or IMUfailkeyword2 in tempsetup:
                               testStatusText_IMU = 'Fail'
                               self.lineEdit_teststatus.setText(testStatusText)
                               self.lineEdit_teststatus.setStyleSheet("background-color:red")
                               self.textBrowser.append('IMU校准失败')
                               ...
                               save_UI_logfile.MyWin.Self_logfile(self, testresult)
                               break
                           elif IMUpasskeyword in tempsetup:
                               testStatusText_IMU = 'PASS'
                               self.lineEdit_teststatus.setText(testStatusText)
                               self.lineEdit_teststatus.setStyleSheet("background-color:green")
                               self.textBrowser.append('IMU校准成功')
                               with open('tmp.txt', 'r') as tmpfile:
