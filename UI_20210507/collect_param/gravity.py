...
import data_process.count_testresult as count_testresult
...
class MyWin(Ui_MainWindow, QMainWindow):
    ...
    def gravity(self):
        ...
        if startResult == QMessageBox.Yes:
            self.textBrowser.append('Gravity test')
            time.sleep(1)
            self.data = subprocess.Popen('../collect_param/Gravity.sh', stdout=subprocess.PIPE,
            ...
            time.sleep(3)
            with open('tmp.txt', 'r') as GravityFile:
                Gravityfailkeyword1 = '1 passed, 1 failed'
                Gravityfailkeyword2 = '0 passed, 2 failed'
                Gravitypasskeyword = '2 passed, 0 failed'
                for tempsetup in GravityFile.readlines():
                    if Gravityfailkeyword1 in tempsetup or Gravityfailkeyword2 in tempsetup:
                        testStatusText_gravity = 'Fail'
                        self.lineEdit_teststatus.setText(testStatusText_gravity)
                        self.lineEdit_teststatus.setStyleSheet("background-color:red")
                        self.textBrowser.append('重力补偿失败')
                        ...
                        break
                    elif Gravitypasskeyword in tempsetup:
                        testStatusText_gravity = 'PASS'
                        self.lineEdit_teststatus.setText(testStatusText_gravity)
                        self.lineEdit_teststatus.setStyleSheet("background-color:green")
                        self.textBrowser.append('重力补偿成功')
                        with open('tmp.txt', 'r') as tmpfile:
